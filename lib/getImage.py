...
def getOauth():
    consumer_key = 'cBahEuAmSGj4QZTKaM9lhnyZg'
    consumer_secret = 'VdgNxnoa4mrDhVI2i5uGmGyg8YGX56Ccc2Uri7fKKJrhB10dnF'
        ...
        try:
            if 'extended_entities' in status._json:
                for i in range(len(status.extended_entities['media'])):
                    ...
                    count += 1
                    sys.stdout.write("\rGet count : %d" % count)
                    sys.stdout.flush()
...
def draw(image_path, name, text):
    ...
    _font = ImageFont.truetype('/Library/Fonts/AdobeFangsongStd-Regular.otf', 12, encoding='utf-8')
